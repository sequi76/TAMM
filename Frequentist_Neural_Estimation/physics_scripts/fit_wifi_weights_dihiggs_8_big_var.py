...
for model in ensemble:
    ...
    with torch.no_grad():
        ...
        # Process signal MSDs (first num_td classes)
        ...
        # Process background MSDs (last num_td classes)
        for i, bkg_data in enumerate(background_fit_data_list):
            llrs_all = model.llrs_all(bkg_data).double()
            llrs_msd_class = llrs_all[:, num_td + i]  # (n_samples,) - extract (num_td + i)-th class
            llrs_per_msd.append(llrs_msd_class)
...
def wifi_loss(w):
    ...
    for k in range(num_classes):
        ...
        data_term = -log_r_k_on_msd.mean() + (torch.exp(-log_r_k_on_msd) - 1).mean()
    ...
    for k in range(num_classes):
        ...
        ref_term = log_r_k_on_ref.mean() + (torch.exp(log_r_k_on_ref) - 1).mean()
        total_loss += ref_term
...
output_path = output_dir + f'wifi_weights_{num_td}_msd_{msds_idx+1}.pt'
