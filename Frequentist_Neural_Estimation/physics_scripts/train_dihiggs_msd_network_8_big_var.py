...
msd_numbers = np.random.choice(good_msds, size=num_td, replace=False)
...
for ii, model in enumerate(ensemble):
    torch.save(model.state_dict(), this_dir + f'varying_msds_big_var/{num_td}_msd_model_{job_idx}_ens_{ii}.pt')
